...
## TL;DR
ElSereno's `iec104` plugin sends a minimal read-only probe on port 2404/tcp
...
## Wire format (summary)
See `internal/protocols/iec104/wire/` for the from-scratch parser.
...
See `internal/protocols/iec104/iec104.go` for the factor defaults.

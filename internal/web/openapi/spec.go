...
func scheduleRequestSchemas() map[string]Schema {
	return map[string]Schema{
		...
		"CreateScheduleRequest": {
			...
			// v2.57+: full sample for code-gen + Swagger UI.
			"example": map[string]any{
				"name": "nightly-fleet",
				"template": map[string]any{
					"input":   "list:fleet.txt",
					"plugins": []string{"modbus", "s7", "bacnet"},
				},
				"cron_expr": "0 2 * * *",
...
func scanScheduleSchema() Schema {
	return Schema{
		...
		"properties": map[string]any{
			...
			"name":                 map[string]any{"type": "string", "example": "nightly-fleet"},
			"template":             map[string]any{"$ref": "#/components/schemas/SubmitRequest"},
			"interval_seconds":     map[string]any{"type": "integer", "example": 3600},
			"cron_expr":            map[string]any{"type": "string", "example": "0 2 * * *"},
			"timezone":             map[string]any{"type": "string", "example": "Europe/Madrid"},
			"enabled":              map[string]any{"type": "boolean", "example": true},
			"operator":             map[string]any{"type": "string", "example": "alice@example.com"},
			...
			"audit_retention_days": map[string]any{"type": "integer", "description": "v1.89+ per-schedule retention override; 0 = inherit global.", "example": 30},
			"tags":                 map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "v2.4+ grouping labels (canonicalised: sorted + deduped).", "example": []string{"prod", "critical"}},
		},
		// v2.57+: top-level example so renderers (Swagger UI,
		// Stoplight, etc.) show a complete sample payload.
		"example": map[string]any{
			"id":                   "abc123def4567890",
			"name":                 "nightly-fleet",
			"template":             map[string]any{"input": "list:fleet.txt", "plugins": []string{"modbus", "s7", "bacnet"}},
...
func jobSchema() Schema {
	return Schema{
		...
		"properties": map[string]any{
			"id":                       map[string]any{"type": "string", "example": "9f8e7d6c5b4a3210"},
			"state":                    map[string]any{"type": "string", "enum": []string{"queued", "running", "completed", "failed", "cancelled"}, "example": "completed"},
			"created_at":               map[string]any{"type": "string", "format": "date-time"},
			"started_at":               map[string]any{"type": "string", "format": "date-time"},
			"finished_at":              map[string]any{"type": "string", "format": "date-time"},
			"input":                    map[string]any{"type": "string", "example": "list:fleet.txt"},
			"plugins":                  map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "example": []string{"modbus", "s7"}},
			"default_port":             map[string]any{"type": "integer", "example": 502},
			"stats":                    map[string]any{"$ref": "#/components/schemas/Stats"},
			"findings_by_plugin":       map[string]any{"type": "object", "additionalProperties": map[string]any{"type": "integer"}, "example": map[string]int{"modbus": 12, "s7": 3}},
			"error":                    map[string]any{"type": "string"},
			"operator":                 map[string]any{"type": "string", "example": "alice@example.com"},
			"triggered_by_schedule_id": map[string]any{"type": "string", "description": "v1.92+ FK to schedules.id when scheduler-fired.", "example": "abc123def4567890"},
		},
		// v2.57+: complete Job sample.
		"example": map[string]any{
			"id":                       "9f8e7d6c5b4a3210",
			"state":                    "completed",
			"created_at":               "2026-05-17T08:00:00Z",
			"started_at":               "2026-05-17T08:00:01Z",
			"finished_at":              "2026-05-17T08:03:42Z",
			"input":                    "list:fleet.txt",
			"plugins":                  []string{"modbus", "s7", "bacnet"},
			"default_port":             0,
			"stats":                    map[string]int{"targets_seen": 145, "targets_scanned": 145, "findings_count": 18},
			"findings_by_plugin":       map[string]int{"modbus": 12, "s7": 3, "bacnet": 3},
			"operator":                 "alice@example.com",
			"triggered_by_schedule_id": "abc123def4567890",
		},
	}
}
